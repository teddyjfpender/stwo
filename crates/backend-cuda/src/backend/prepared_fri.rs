...
impl<'a> PreparedFriGraph<'a> {
    ...
    /// Execute exactly one challenge-bounded fold round and, unless it reached
    /// the final layer, commit its output tree. The alpha powers are generated by
    /// repeated squaring exactly like `squared_alpha_powers`.
    pub fn launch_round(
        &self,
        round_index: usize,
        folding_alpha: SecureField,
    ) -> Result<Option<usize>, PreparedFriError> {
        self.launch_round_folds_only(round_index, folding_alpha)?;
        let round = self
            .rounds
            .get(round_index)
    ...
    pub fn launch_round_folds_only(
        &self,
        round_index: usize,
        folding_alpha: SecureField,
    ) -> Result<(), PreparedFriError> {
        let round = self
            ...
            .get(round_index)
            .ok_or(PreparedFriError::InvalidRoundIndex(round_index))?;
        let mut alpha = folding_alpha;
        for launch in &round.folds {
            self.launch_fold(*launch, alpha)?;
            alpha *= alpha;
        }
        Ok(())
    }
    ...
    fn launch_fold(&self, launch: FoldLaunch, alpha: SecureField) -> Result<(), PreparedFriError> {
        ...
        let output_ptrs = launch
            ...
            .as_u32_ptr()
            .cast::<*mut u32>();
        let raw_alpha = CudaSecureField::from(alpha).into_raw();
