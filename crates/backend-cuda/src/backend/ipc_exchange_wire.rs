...
pub const IPC_EXCHANGE_DESCRIPTOR_BYTES: usize = 280;
pub const IPC_EXCHANGE_CLOSE_RECEIPT_BYTES: usize = 88;
...
impl CudaDeviceUuid {
    pub const fn from_bytes(bytes: [u8; CUDA_DEVICE_UUID_BYTES]) -> Self {
        Self(bytes)
    }
...
impl IpcExchangeDescriptor {
    ...
    pub fn encode(&self) -> [u8; IPC_EXCHANGE_DESCRIPTOR_BYTES] {
        ...
        out[88..152].copy_from_slice(&self.memory_handle);
        out[152..216].copy_from_slice(&self.ready_event_handle);
        out[216..280].copy_from_slice(&self.consumed_event_handle);
        out
    }
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, IpcExchangeError> {
        ...
        let key = decode_key(&bytes[..88], DESCRIPTOR_MAGIC, false)?.0;
        ...
        memory_handle.copy_from_slice(&bytes[88..152]);
        let mut ready_event_handle = [0u8; CUDA_IPC_HANDLE_BYTES];
        ready_event_handle.copy_from_slice(&bytes[152..216]);
        let mut consumed_event_handle = [0u8; CUDA_IPC_HANDLE_BYTES];
        consumed_event_handle.copy_from_slice(&bytes[216..280]);
        Ok(Self {
            key,
            allocation_bytes,
            memory_handle,
